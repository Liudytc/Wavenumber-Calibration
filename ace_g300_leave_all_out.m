...
%% Parameters 
wavenumber_NIST=([213.3,329.2,465.1,504.0,651.6,797.2,857.9,968.7,1105.5,1168.5,1236.8,1323.9,1371.5,1515.1,1561.5,1648.4,2931.1,3064.6,3102.4,3326.6]);% Material related wavenumber reference
laser=532e-9;
current_grating=300;
d=1e-3/current_grating;
T = 26.00e-6;
fy_centre = 21.88;%degree
alfa=fy_centre/2;
rotate_centre=5;
rotate_range = 4;
rotate_n=501;
...
peak_positions_subpixel_NIST_temp=zeros(100,size(wavenumber_NIST,2));
ROTATE1=zeros(1,100);ALFA1=zeros(1,100); D1=zeros(1,100); Laser1=zeros(1,100); 
ROTATE2=zeros(1,100);ALFA2=zeros(1,100); D2=zeros(1,100); Laser2=zeros(1,100); 
ROTATE3=zeros(1,100);ALFA3=zeros(1,100); D3=zeros(1,100); Laser3=zeros(1,100); 
First=zeros(100,2);first_pixel=zeros(100,size(wavenumber_NIST,2));First_inv=zeros(100,2);
Second=zeros(100,3);second_pixel=zeros(100,size(wavenumber_NIST,2));Second_inv=zeros(100,3);
Third=zeros(100,4);third_pixel=zeros(100,size(wavenumber_NIST,2));Third_inv=zeros(100,4);
...
parfor i=1:100
    for laser=laser_centre-laser_range/2:laser_step_size:laser_centre+laser_range/2
        ...
        peak_positions_subpixel_NIST_temp(i,:) =-(f/T)*tand(-alfa + rotate+asind(-laser./(d-1e2.*wavenumber_NIST.*laser.*d)-sind(-rotate-alfa)))+c(i);
        first=polyfit(peak_positions_subpixel_NIST_temp(i,:),peak_positions_subpixel(i,:),1);
        first_inv=[1/first(1),-first(2)/first(1)];
        X_first=polyval(first_inv,peak_positions_subpixel(i,:));
        temp_wavenum1 = (1./(d./laser.*(sind(atand((c(i).*T-X_first.*T)./f)+alfa-rotate)+sind(-alfa-rotate)))+1).*1e-2./laser;
        Er1_inv=mean(abs(temp_wavenum1-wavenumber_NIST));
        Er1_raw=abs(temp_wavenum1-wavenumber_NIST);
        first_all=polyval(first,peak_positions_subpixel_NIST_temp(i,:));
        SD=std(temp_wavenum1-wavenumber_NIST);
        RMSE=sqrt(mean((temp_wavenum1-wavenumber_NIST).^2));
        if Er1_inv<MIN1(i)
        MIN1(i) =Er1_inv;
        ROTATE1(i)=rotate;
        ALFA1(i)=alfa;
...
for i=1:100
temp1=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,1);
temp2(i,:)=polyval(temp1,peak_positions_subpixel(i,:));
error2(i,:) =mean(abs(temp2(i,:)-wavenumber_NIST)); 

temp3=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,2);
temp4(i,:)=polyval(temp3,peak_positions_subpixel(i,:));    
error3(i,:) =mean(abs(temp4(i,:)-wavenumber_NIST));

temp5=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,3);
temp6(i,:)=polyval(temp5,peak_positions_subpixel(i,:));
error4(i,:) =mean(abs(temp6(i,:)-wavenumber_NIST).^2);

temp7=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,4);
temp8(i,:)=polyval(temp7,peak_positions_subpixel(i,:));
error5(i,:) =mean(abs(temp8(i,:)-wavenumber_NIST));

temp9=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,5);
temp10(i,:)=polyval(temp9,peak_positions_subpixel(i,:));
error6(i,:) =mean(abs(temp10(i,:)-wavenumber_NIST));

temp11=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,6);
temp12(i,:)=polyval(temp11,peak_positions_subpixel(i,:));
error7(i,:) =mean(abs(temp12(i,:)-wavenumber_NIST));

temp13=polyfit(peak_positions_subpixel(i,:),wavenumber_NIST,7);
temp14(i,:)=polyval(temp13,peak_positions_subpixel(i,:));
error8(i,:) =mean(abs(temp14(i,:)-wavenumber_NIST));
end
for i=1:100
...
STD_e1=mean(SD1);STD_e2=0;STD_e3=0;STD_o1=mean(std_orignial1);STD_o2=mean(std_orignial2);STD_o3=mean(std_orignial3);
...
RMSE_e1=mean(RMSE1);RMSE_e2=0;RMSE_e3=0;RMSE_o1=mean(rmse_orignial1);RMSE_o2=mean(rmse_orignial2);RMSE_o3=mean(rmse_orignial3);
...
STD_ace_g300_all_peaks=[STD_e1,STD_o1,STD_o2,STD_o3,STD_o4,STD_o5,STD_o6,STD_o7];
RMSE_ace_g300_all_peaks=[RMSE_e1,RMSE_o1,RMSE_o2,RMSE_o3,RMSE_o4,RMSE_o5,RMSE_o6,RMSE_o7];
MAE_ace_g300_all_peaks=[mean(MIN1),mean(error2),mean(error3),mean(error4),mean(error5),mean(error6),mean(error7),mean(error8)];
